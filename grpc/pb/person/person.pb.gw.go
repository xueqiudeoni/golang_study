...
var (
	filter_SearchService_Search_0 = &utilities.DoubleArray{Encoding: map[string]int{"name": 0, "age": 1, "Mbody": 2}, Base: []int{1, 1, 2, 6, 0, 0, 4, 0}, Check: []int{0, 1, 1, 1, 2, 3, 4, 7}}
)
...
func request_SearchService_Search_0(ctx context.Context, marshaler runtime.Marshaler, client SearchServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	...
	val, ok = pathParams["Mbody.name"]
	if !ok {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "missing parameter %s", "Mbody.name")
	}

	err = runtime.PopulateFieldFromPath(&protoReq, "Mbody.name", val)

	if err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "type mismatch, parameter: %s, error: %v", "Mbody.name", err)
	}
...
var (
	pattern_SearchService_Search_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 1, 5, 2, 1, 0, 4, 1, 5, 3, 1, 0, 4, 1, 5, 4}, []string{"api", "person", "name", "age", "Mbody.name"}, "", runtime.AssumeColonVerbOpt(true)))
)
